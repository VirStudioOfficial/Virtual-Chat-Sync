...
function longestTextOverlap(previousText, nextText) {
    const previous = String(previousText || '');
    const next = String(nextText || '');
    if (!previous || !next) return 0;
    const max = Math.min(previous.length, next.length, 1200);
    const suffix = previous.slice(-max);
    const lowerSuffix = suffix.toLocaleLowerCase();
    const lowerNext = next.toLocaleLowerCase();
    for (let len = max; len >= 24; len--) {
        if (lowerSuffix.slice(-len) === lowerNext.slice(0, len)) return len;
    }
    // برای تکرارهای کوتاه‌تر هم یک حداقل محافظ داریم؛ این فقط وقتی متن دقیقاً
    // از ابتدای پاسخ قبلی دوباره شروع شده باشد فعال می‌شود.
    for (let len = Math.min(max, 23); len >= 8; len--) {
        if (lowerSuffix.slice(-len) === lowerNext.slice(0, len)) return len;
    }
    return 0;
}
...
async function streamRoundWithRecovery({ contents, modelName, key, externalSignal, firstByteTimeoutMs, includeTools, onText, searchIntent }) {
    ...
    for (let attempt = 0; ; attempt++) {
        // در round اول، متن را زنده پخش می‌کنیم. در recovery باید کل round جدید را
        // موقتاً نگه داریم تا قبل از نمایش، overlap ابتدای آن با انتهای پاسخ قبلی
        // حذف شود؛ وگرنه Gemini ممکن است چند جمله‌ی قبلی را دوباره تولید کند.
        let roundStreamText = '';
        const round = await streamOneGeminiRound({
            ...
            onText: piece => {
                if (!piece) return;
                roundStreamText += piece;
                if (attempt === 0) {
                    try { onText(piece); } catch (_) {}
                }
            },
            searchIntent: attempt === 0 ? searchIntent : false
        });

        const previousVisible = combinedText;
        let visibleRoundText = round.text;
        if (attempt > 0 && visibleRoundText) {
            const overlap = longestTextOverlap(previousVisible, visibleRoundText);
            if (overlap > 0) {
                visibleRoundText = visibleRoundText.slice(overlap);
                console.warn(`[shared-chats] recovery overlap removed: ${overlap} chars`);
            }
            if (visibleRoundText) {
                try { onText(visibleRoundText); } catch (_) {}
            }
        }

        combinedText += visibleRoundText;
